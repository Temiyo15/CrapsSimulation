...
public class PlayWithInheritance
{
	...
	public static void main(String[] args)
	{
		...
		else if (RUN_EXAMPLE == 2)
		{
			...
			StdOut.println(singledie1.toString());

			StdOut.print("toString() automatically called when reference treated as String: ");
			StdOut.println(singledie1.toString());
			StdOut.println("Concatenating a string to a reference: " + singledie1.toString());
		}
		else if (RUN_EXAMPLE == 3)
		{
			...
			StdOut.println("die1's toString(): '" + singledie1 + "'");
		...
		else if (RUN_EXAMPLE == 6)
		{
			...
			Dice dice = new Dice(singledie1, crooked1); // we'll try substituting here

			dice.roll();

			int result = dice.getLastRoll();

			StdOut.printf("Roll of '%s' and '%s' is: %d.\n", singledie1, crooked1, result);
		}
